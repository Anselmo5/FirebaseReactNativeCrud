...
const Listagem = () => {
  ...
  const [editId, setEditId] = useState(""); // edição-
  let [editState, setEditState] = useState("none")// edição 
  const router = useRouter()

  useEffect(() => {  // o useEffct fara a função de mapear todos os dados onde com snapshot pegara todos os dados da collection pessoas
    const unsubscribe = firebase.firestore().collection('Pessoas').onSnapshot((snapshot) => {
      const data = snapshot.docs.map((doc) => ({ ...doc.data(), id: doc.id })); // cada doc possue um id
      setPessoas(data);
    });
    return () => unsubscribe();
  }, []); // array fara a função roda uma unica fez
  
  const excluirPessoa = (id) => { // função que exclue os dados
  ...
  const showEdit = (id) => {  //active
    setEditState("flex")
    setEditId(id)
  }
// essas duas funções são a parte da style de edição da lista crida, 
  const closeEdit = () => {  //none
    setEditState("none")
    setEditId("")
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList // Flatlist esta fazendo o papel de renderização de componentes criando uma lista
        data={pessoas}
        renderItem={renderPessoa}
        keyExtractor={(item) => item.id} // cada componente que criamos possue sua propria key, estamos aqui definindo que cada item tera uma key cendo essa key unica, e gente pega essa key atraves do id criado
